...
| 番組一覧の表示 | フォロー中一覧は**フォローした順**（新しく追加したものが上）に並べる。各行に、表示順 / フィードで一番新しい回の公開日 / 再生済みと全件を出す。新着を強調する印は付けず、どの番組も同じ見た目にする。更新日と全件は番組を開いたときに `follows` へ控えた値、再生済み件数は `episodes` の索引から数えるので、ホームの描画でフィードは取りに行かない |
...
- **番組一覧の並び順**: フォローした順（新しく追加したものが上、`js/db.js` の `listFollows`）。
  更新の新しい順にすると、聴く前から新着のある番組が上位に来てしまい「新着を強調しない・
  急かさない」方針と食い違う（一度も開いていない番組は更新日も持たない）。題名順は読みを
  持っておらず、漢字の題名で並びの根拠が出せないため採らない。番組を追加したとき以外は
  入れ替わらないので、探す場所が毎回変わらない。`followedAt` を持たない古い記録は
  題名 → `feedUrl` の順に置き、描き直しても同じ場所に落ち着かせる
