...
| 新着の自動取り込み | 番組を開くとキャッシュから即座に表示し、裏で更新の有無だけを問い合わせる。新しくなっていたときだけ一覧を差し替えて知らせる |
...
中継に `etag` / `modified` を渡すと、配信元へ `If-None-Match` / `If-Modified-Since` を付けて
問い合わせ、変わっていなければ本文なしで返す（ブラウザのキャッシュ機構と紛れないよう
304 ではなく 204 を使う）。印を返さない配信元もあるので、その場合は取得した中身を
突き合わせて変化の有無を判定する。
