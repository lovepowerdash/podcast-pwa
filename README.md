...
- **ロック画面／コントロールセンター／Bluetoothイヤホンの再生ボタンが効かない条件**
  （`player.resume()` で全部拾う）。押しても何も起きない状態が4つあり、
  いずれも `audio.play()` を呼ぶだけでは音が出ない。
  ...
  4. `play()` が握り潰された — 拒否も `error` も返らないまま鳴り始めない。
     画面を消したまま一時停止したあと再生を押すと起きる（下記）
