...
| 番組一覧の表示 | フォロー中一覧の各行に、表示順 / フィードで一番新しい回の公開日 / 再生済みと全件を出す。新着を強調する印は付けず、どの番組も同じ見た目にする。更新日と全件は番組を開いたときに `follows` へ控えた値、再生済み件数は `episodes` の索引から数えるので、ホームの描画でフィードは取りに行かない |
