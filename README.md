...
| 引っ張って更新 | エピソード一覧とホームの両方で、上端から下に引くと新着を確かめ直す。エピソード一覧は開いたときと同じ差分の確認、ホームはフォロー中の全番組をまとめて確認（3本ずつ流す）。ホームは普段いっさい通信しないので、まとめて確かめられるのはこの操作だけ。見つかっても知らせは出さず、一覧と更新日が変わるだけ。手応えは iOS の標準に合わせ、一覧そのものが指に付いて下がり、引くほど重くなる（`pullResistance`）。中継が答えを返せなかったときは、キャッシュの期限内でも通常の取得経路で取り直す（「確かめられなかった」を「変わっていない」として扱うと、新着があっても出てこない） |
...
- **ロック画面／コントロールセンターの再生ボタンが効かない条件**（`player.resume()` で全部拾う）。
